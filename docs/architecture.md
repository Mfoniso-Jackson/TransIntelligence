...
Observation retrieval is handled by `ObservationStore` and `ObservationQuery`, keeping storage concerns separate from reasoning algorithms while providing deterministic in-memory querying for tests and examples.
